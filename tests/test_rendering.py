...
class RenderingTest(TestCase):
    def setUp(self):
        # Create a components directory in the test environment
        self.components_dir = 'components'
        os.makedirs(self.components_dir, exist_ok=True)

        self.default_template_path = os.path.join(self.components_dir, 'note.html')
        with open(self.default_template_path, 'w') as f:
            f.write('<h1>{{ object.content }}</h1>')

        self.card_template_path = os.path.join(self.components_dir, 'note_card.html')
        with open(self.card_template_path, 'w') as f:
            f.write('<h2>{{ object.content }}</h2>')

        self.exposed_template_path = os.path.join(self.components_dir, 'exposedmodel.html')
    ...
    def test_render_component_with_exposed_data(self):
        ...
        # Check that the x-data attribute is correctly formed
        self.assertIn('x-data=\'byrdieComponent({', rendered_html)
        self.assertIn('"pk": {}'.format(instance.pk), rendered_html)
        self.assertIn('"name": "Test"', rendered_html)
        self.assertIn('"value": 42', rendered_html)
        self.assertIn('"exposed_methods": ["double"]', rendered_html)
    ...
    def test_render_component_direct_call(self):
        ...
        rendered_html = render_component(instance)
        self.assertIn('>Test Content</h1>', rendered_html)
    ...
    def test_component_tag(self):
        ...
        self.assertIn('>Tag Test</h1>', rendered_html)
    ...
    def test_named_component(self):
        ...
        context = Context({'instance': instance})
        rendered_html = template.render(context)
        self.assertIn('>Card Test</h2>', rendered_html)
    ...
    def test_render_component_not_found(self):
        ...
        rendered_html = render_component(instance)
        self.assertEqual(rendered_html.strip(), '<!-- Component template not found: components/anothermodel.html -->')
